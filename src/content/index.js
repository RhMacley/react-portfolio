export default {
  nav: {
    logo: "Portfólio",
    links: [
      { text: "Skills", to: "mystack" },
      { text: "Contact", to: "mycontact" },
    ],
  },
  header: {
    img: process.env.PUBLIC_URL + "/assets/code4.png",
    imgPlaceholder:
      "data:image/jpeg;base64,",
    text: ["Hello, how are you?", "I'm Rhanon Macley", "I am "],
    typical: ["Software Engineer 💻", 2000, "Developer 👨‍💻", 2000],
    btnText: "Find out more",
  },
  work: {
    title: "Projects",
    img: process.env.PUBLIC_URL + "/assets/project-mobile.png",
    imgPlaceholder:
      "data:image/png;base64,",
    projectName: "Sleeping",
    desc: "peaceful, and tranquil mean quiet and free from disturbance. calm is used when someone is not excited or upset even when there is cause for it. They stayed calm during the fire. peaceful is used when someone or something has reached a quiet state after some period of disturbance.",
  },
  stack: {
    title: "Stack",
    tech: [
      {
        img: process.env.PUBLIC_URL + "/assets/html-css2.png",
        alt: "html/css",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/node.png",
        alt: "nodejs",
      },
      {
        img: process.env.PUBLIC_URL + "/logo512.png",
        alt: "reactjs",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/ts.png",
        alt: "typescript",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/nestjs.png",
        alt: "nestjs",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/mongo.png",
        alt: "mongo",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/mysql.png",
        alt: "mysql",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/aws.png",
        alt: "aws",
      },
      {
        img: process.env.PUBLIC_URL + "/assets/rabbitmq.png",
        alt: "rabbitmq",
      },
    ],
    desc: `
    I am a professional passionate about what I do, I crossed an entire country to fulfill my dream of working in the technology field. I'm looking for a job as a software engineer, in a company where I can have an opportunity to amplify my knowledge and contribute to its growth .`,
  },
  contact: {
    title: "Contact",
    desc: "Below are the links where you can get to know me a little more:",
    img: process.env.PUBLIC_URL + "/assets/code4.png",
    imgPlaceholder:
      "data:image/jpeg;base64,",
  },
};
